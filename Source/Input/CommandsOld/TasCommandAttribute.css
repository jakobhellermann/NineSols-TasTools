...
public class TasCommandAttribute : Attribute {
    private static readonly IDictionary<TasCommandAttribute, MethodInfo> MethodInfos =
        new Dictionary<TasCommandAttribute, MethodInfo>();
    ...
    public bool IsName(string name) {
        var result = Name.Equals(name, StringComparison.InvariantCultureIgnoreCase);
        if (AliasNames == null || AliasNames.Length == 0)
            return result;
        else
            return result || AliasNames.Any(aliasName =>
                aliasName.Equals(name, StringComparison.InvariantCultureIgnoreCase));
    }

    [Initialize]
    private static void CollectMethods() {
        MethodInfos.Clear();
        var methodInfos = typeof(TasCommandAttribute).Assembly.GetTypes().SelectMany(type =>
                type
                    .GetMethods(BindingFlags.Static | BindingFlags.Public | BindingFlags.NonPublic))
            .Where(info => info.GetCustomAttributes<TasCommandAttribute>().Any());
        foreach (var methodInfo in methodInfos) {
            var tasCommandAttributes = methodInfo.GetCustomAttributes<TasCommandAttribute>();
            foreach (var tasCommandAttribute in tasCommandAttributes) MethodInfos[tasCommandAttribute] = methodInfo;
        }
    }
