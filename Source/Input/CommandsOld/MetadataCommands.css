...
//         Everest.Events.Level.OnComplete -= UpdateChapterTime;
    ...
    private static void LevelOnBegin() {
        StartFileTime();
    }

    private static void LevelOnUpdateTime() {
        StartFileTime();
    }

    private static void StartFileTime() {
        //if (Manager.Running && TasStartFileTime == null) TasStartFileTime = SaveData.Instance?.Time;
    }
    ...
    [TasCommand("MidwayChapterTime", AliasNames = new[] { "MidwayChapterTime:", "MidwayChapterTime：" },
        CalcChecksum = false)]
    private static void MidwayChapterTimeCommand() {
        /*if (!Manager.Running || Engine.Scene is not Level level || !level.Session.StartedFromBeginning) return;
        UpdateAllMetadata("MidwayChapterTime",
            _ => GameInfo.GetChapterTime(level),
            command => Manager.Controller.CurrentCommands.Contains(command));*/
    }

    private static void UpdateChapterTime() {
        /*if (!Manager.Running || !level.Session.StartedFromBeginning) return;
    ...
    public static void UpdateRecordCount(InputController inputController) {
        UpdateAllMetadata(
            ...
            command => int.TryParse(command.Args.FirstOrDefault() ?? "0", out var _));
    }

    private static void UpdateAllMetadata(string commandName, Func<Command, string> getMetadata,
        Func<Command, bool> predicate = null) {
        var inputController = Manager.Controller;
        var tasFilePath = InputController.TasFilePath;
        ...
        var updateLines = metadataCommands.Where(command => {
            var metadata = getMetadata(command);
            if (metadata is null || metadata.Length == 0) return false;

            if (command.Args.Length > 0 && command.Args[0] == metadata) return false;

            return true;
        }).ToDictionary(command => command.StudioLineNumber,
            command => $"{command.Attribute.Name}: {getMetadata(command)}");

        if (!updateLines.Any()) return;

        string[] allLines = File.ReadAllLines(tasFilePath);
        var allLinesLength = allLines.Length;
        foreach (var lineNumber in updateLines.Keys)
            if (lineNumber >= 0 && lineNumber < allLinesLength)
                allLines[lineNumber] = updateLines[lineNumber];

        var needsReload = Manager.Controller.NeedsReload;
