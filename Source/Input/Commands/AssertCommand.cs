...
public static class AssertCommand {
    private class Meta : ITasCommandMeta {
        public string Insert =>
            $"Assert{CommandInfo.Separator}[0;Condition]{CommandInfo.Separator}\"[1;Expected]\"{CommandInfo.Separator}\"[2;Actual]\"";
        ...
        public IEnumerator<CommandAutoCompleteEntry> GetAutoCompleteEntries(string[] args, string filePath,
            ...
            foreach (var mode in Enum.GetValues(typeof(AssertCondition)))
                yield return new CommandAutoCompleteEntry
                    { Name = mode.ToString(), Extra = "Condition", HasNext = true };
        }
    }
    ...
    private static void Assert(CommandLine commandLine, int studioLine, string filePath, int fileLine) {
        ...
        if (Command.Parsing) {
            ...
            else if (args.Length < 3 || args[2].IsEmpty()) AbortTas($"{prefix}Lack of actual value");
        } else {
            var condition = Enum.Parse<AssertCondition>(args[0], true); // Must succeed, since this was checked in Parse
            var expected = args[1];
            var actualTemplate = args[2];
            var failureMessage = args.Length >= 4 ? args[3] : null;
